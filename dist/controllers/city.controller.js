...
exports.importCities = exports.getById = exports.deleteById = exports.updateById = exports.getCity = exports.addCity = void 0;
...
const addCity = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const CityNameCheck = yield City_model_1.City.findOne({
        ...
        const newEntry = new City_model_1.City(req.body).save();
        if (!newEntry) {
...
const getCity = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        let cityCount = yield City_model_1.City.find(query).countDocuments();
        let pageValue = req.query.page ? parseInt(`${req.query.page}`) : 1;
        let limitValue = req.query.perPage ? parseInt(`${req.query.perPage}`) : 2000;
        CityArr = yield City_model_1.City.find(query)
        ...
        for (let city of CityArr) {
            // if (city.countryId) {
            //   city.countryObj = await Country.findById(city.countryId).exec();
            // }
            if (city.stateId) {
                city.stateObj = yield State_model_1.State.findById(city.stateId).exec();
            }
        }
        console.log(CityArr.length);
        res.status(200).json({ message: "getCity", data: CityArr, cityCount: cityCount, success: true });
    }
    catch (err) {
        next(err);
    }
...
const updateById = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const CityObj = yield City_model_1.City.findById(req.params.id).lean().exec();
        if (!CityObj) {
        ...
        yield City_model_1.City.findByIdAndUpdate(req.params.id, req.body).exec();
        res.status(200).json({ message: "City Updated", success: true });
    }
    catch (err) {
        next(err);
    }
...
const deleteById = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const CityObj = yield City_model_1.City.findByIdAndDelete(req.params.id).exec();
        if (!CityObj)
            throw { status: 400, message: "City Not Found" };
        res.status(200).json({ message: "City Deleted", success: true });
    }
    catch (err) {
        next(err);
    }
...
const getById = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        let CityObj = yield City_model_1.City.findById(req.params.id).lean().exec();
        if (!CityObj)
            throw { status: 400, message: "City Not Found" };
        if (CityObj.countryId) {
            CityObj.countryObj = yield country_model_1.Country.findById(CityObj.countryId).exec();
        }
        if (CityObj.stateId) {
            CityObj.stateObj = yield State_model_1.State.findById(CityObj.stateId).exec();
        }
        res.status(200).json({ message: "City Found", data: CityObj, success: true });
    }
    catch (err) {
        next(err);
    }
...
const importCities = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        let country = yield country_model_1.Country.findOne({ name: new RegExp("india", "i") }).exec();
        if (!country) {
        ...
        for (const el of cityArr) {
            let stateObj = yield State_model_1.State.findOne({ name: new RegExp(`${el.state}`, "i") }).exec();
            if (stateObj) {
                yield new City_model_1.City({ name: el.name, stateId: stateObj._id, countryId: country._id }).save();
            }
            else {
                let tempStateObj = yield new State_model_1.State({ name: el.state, countryId: country._id }).save();
                yield new City_model_1.City({ name: el.name, stateId: tempStateObj._id, countryId: country._id }).save();
            }
        }
