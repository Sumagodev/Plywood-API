...
export const addCity = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const CityNameCheck = await City.findOne({
      name: new RegExp(`^${req.body.name}$`, "i"),
    }).exec();
    if (CityNameCheck) throw new Error("Entry Already exist please change brand name ");

    if (req.body.image) {
      req.body.image = await storeFileAndReturnNameBase64(req.body.image);
    }
    const newEntry = new City(req.body).save();
...
export const getCity = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    let cityCount = await City.find(query).countDocuments();

    let pageValue = req.query.page ? parseInt(`${req.query.page}`) : 1;
    let limitValue = req.query.perPage ? parseInt(`${req.query.perPage}`) : 2000;

    CityArr = await City.find(query)
    ...
    for (let city of CityArr) {
      ...
      // }
      if (city.stateId) {
        city.stateObj = await State.findById(city.stateId).exec();
      }
    }
...
export const updateById = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const CityObj = await City.findById(req.params.id).lean().exec();
    if (!CityObj) {
    ...
    await City.findByIdAndUpdate(req.params.id, req.body).exec();
...
export const deleteById = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const CityObj = await City.findByIdAndDelete(req.params.id).exec();
    if (!CityObj) throw { status: 400, message: "City Not Found" };
...
export const getById = async (req: Request, res: Response, next: NextFunction) => {
  try {
    let CityObj: any = await City.findById(req.params.id).lean().exec();
    if (!CityObj) throw { status: 400, message: "City Not Found" };
    if (CityObj.countryId) {
      CityObj.countryObj = await Country.findById(CityObj.countryId).exec();
    }
    if (CityObj.stateId) {
      CityObj.stateObj = await State.findById(CityObj.stateId).exec();
    }
...
export const importCities = async (req: Request, res: Response, next: NextFunction) => {
  try {
    let country = await Country.findOne({ name: new RegExp("india", "i") }).exec();
    if (!country) {
    ...
    for (const el of cityArr) {
      let stateObj = await State.findOne({ name: new RegExp(`${el.state}`, "i") }).exec();
      if (stateObj) {
        await new City({ name: el.name, stateId: stateObj._id, countryId: country._id }).save();
      } else {
        let tempStateObj = await new State({ name: el.state, countryId: country._id }).save();
        await new City({ name: el.name, stateId: tempStateObj._id, countryId: country._id }).save();
      }
    }
